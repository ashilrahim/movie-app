        ...
        <div className="bg-[#0f0d23] text-white max-w-6xl mx-auto p-6 rounded-2xl shadow-xl">
          ...
          <div className="grid md:grid-cols-3  mt-6">
            <img
              ...
              alt="Poster"
              className="rounded-lg shadow-lg object-cover h-[250px] md:h-[350px] md:col-span-1"
            />
            <div className="relative md:col-span-2">
              ...
              <button className="absolute flex items-end inset-0 justify-start rounded-lg  p-4">
                ...
                <span className="ml-2">Trailer · 0:31</span>
              </button>
            </div>
          </div>
          ...
          {genres && genres.length > 0 && (
            <div className="flex gap-2 mt-6 flex-wrap">
              {genres.map((genre) => (
                <span
                  key={genre.id}
                  className="bg-purple-800 px-3 py-1 rounded-lg text-sm"
                >
                  {genre.name}
                </span>
              ))}
          ...
          spoken_languages?.length > 0 ? (
            <div className="grid sm:grid-cols-1 mt-6 text-sm gap-3">
              <p className="font-bold text-base text-gray-200">
                <span className="text-lg font-semibold text-white">Release date: </span>
                {release_date.split("-")[0]}
              </p>
              <p className="font-bold text-base text-gray-200">
                <span className="text-lg font-semibold text-white">Countries: </span>
                {production_countries
